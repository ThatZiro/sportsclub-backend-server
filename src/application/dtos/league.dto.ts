...
export const CreateLeagueDTOSchema = z.object({
  ...
  slug: z.string()
    ...
    .regex(/^[a-z0-9-]+$/, 'League slug must contain only lowercase letters, numbers, and hyphens'),
  season: z.string().max(50, 'Season must be less than 50 characters').optional(),
 ...
 * Update league DTO
 */
export const UpdateLeagueDTOSchema = z.object({
  name: z.string().min(1, 'League name is required').max(100, 'League name must be less than 100 characters').optional(),
  slug: z.string()
    .min(1, 'League slug is required')
    .max(50, 'League slug must be less than 50 characters')
    .regex(/^[a-z0-9-]+$/, 'League slug must contain only lowercase letters, numbers, and hyphens')
